...
impl Transaction {
    ...
    /// Creates a new UTXO transaction.
    /// Returns a `Transaction` object.
    ...
    /// Creates a trimmed copy of the transaction.
    /// Returns a `Transaction` object.
    ...
    ///
    /// Returns `true` if it is a coinbase transaction, `false` otherwise.
    ...
    /// Returns the transaction ID as a `Vec<u8>`.
    ...
    /// Returns the serialized transaction as a `Vec<u8>`.
